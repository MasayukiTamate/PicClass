...
def open_visual_sort_window(target_path, app_state, move_callback, refresh_callback, parent=None):
    ...
    try:
        from lib.GazoToolsGUI import VisualSortWindow
        import shutil
        from send2trash import send2trash
        
        # ロジックコールバック (GUIからのアクションを受け取る)
        ...
        def gui_logic_callback(action_type, file_list, win_ref, dest_path=None):
            ...
            if action_type == "move":
                # 登録フォルダへの移動
                if not dest_root:
                    messagebox.showerror("エラー", "移動先（登録フォルダ）が設定されていないのじゃ。\nメイン画面の「Drop Folder Here」にフォルダを登録してほしいのじゃ。")
                    return
                ...
                try:
                     for f in file_list:
                         if move_callback:
                             try:
                                 # move_callback(src, dest, refresh=False)
                ...
                messagebox.showinfo("完了", f"{success_count}ファイルを移動したのじゃ")
                try:
                    win_ref.destroy()
                except:
                    pass
                ...
                messagebox.showinfo("完了", f"{success_count}ファイルをコピーしたのじゃ")

            elif action_type == "trash":
                # ゴミ箱へ送る
                if messagebox.askyesno("確認", f"{len(file_list)}ファイルをゴミ箱に捨ててもよいかの？"):
                    for f in file_list:
                        try:
                            # send2trashを使って安全に削除
                            send2trash(f)
                    ...
                    messagebox.showinfo("完了", f"{success_count}ファイルをゴミ箱に送ったのじゃ")
