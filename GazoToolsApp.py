...
# 実行ファイル化されたとき、外部のPythonスクリプト(libフォルダなど)を優先して読み込む設定
# これにより、EXE化後もスクリプトファイルの差し替えによるアップデートが可能になるのじゃ。
...
# ロジックモジュールのインポート
from GazoToolsLogic import load_config, save_config, HakoData, GazoPicture, calculate_file_hash, VectorBatchProcessor, save_ratings, save_tags, calculate_window_layout, open_visual_sort_window
...
def run_visual_sort():
    """AI Visual Sort を起動するのじゃ。現在表示中の画像があればそれをターゲットにするのじゃ。"""
    try:
        # 現在表示中の画像を取得
        target_path = None
        if hasattr(GazoControl, '_current_image_path') and GazoControl._current_image_path:
             target_path = GazoControl._current_image_path
        
        # なければ現在選択中の画像（ファイルリストから）？
        # まだ何も表示していない場合はユーザーに選択させる
