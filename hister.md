...
- **機能追加**: 「スマート移動機能（類似画像同時移動）」を実装しました。
  ...
  - **機能追加 (2026/01/07)**:
    - **AI Visual Sort (視覚的仕分け)** を実装しました。
      - ターゲット画像と類似した画像を一覧表示し、直感的に仕分け（移動/コピー/削除）を行えるGUIツールです。
      - `VisualSortWindow` クラスを `lib/GazoToolsGUI.py` に新規作成。
      - `GazoToolsLogic.py` に `open_visual_sort_window` 関数を追加し、ロジックとGUIを連携。
      - `GazoToolsApp.py` の「ツール」メニューおよびコンテキストメニューから起動可能にしました。
      - 類似度判定の閾値をスライダーでリアルタイムに変更可能。
      - 「移動」「コピー」「ゴミ箱」の各アクションに対応。
      - `manual.md` および `manual.html` に機能説明を追加。

## 2026/02/10

- **ドキュメント作成**: 部分アップデート（スクリプト差し替えによる更新）の手順を解説した `manual_update.md` および `manual_update.html` を作成しました。
  - `GazoTools.exe` を置き換えずに、`GazoToolsLogic.py` や `lib` フォルダのみを更新する方法を記載。
